...
def read_temperature_sensors(external_sensor):
    ...
    try:
        ...
        both_responses = res.split(',')
        old_sensor = float(both_responses[0].strip())
        new_sensor = float(both_responses[1].strip())
        return (old_sensor, new_sensor)
    except Exception:
...
def read_temperatures(tempdeck, external_sensor=None):
    ...
    external_temp, new_temp = read_temperature_sensors(external_sensor);
    write_line_to_file(
        'Sensors: old={0} new={1}'.format(external_temp, new_temp))
    return (tempdeck.temperature, external_temp, new_temp)
...
def get_sensors_delta(tempdeck, external_sensor):
    tempdeck_temp, external_temp, new_temp = read_temperatures(tempdeck, external_sensor)
